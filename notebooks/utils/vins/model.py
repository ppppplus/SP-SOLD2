...
from .utils.line_detection import LineSegmentDetectionModule
...
class SPSOLD2ExtractModel(BaseExtractModel):
    ...
    def extract(self, img):
        ...
        lines, lines_desc = self.reprocess_lines(junctions, heatmap, coarse_desc)
        # print("superpoint run time:%f", end_time-start_time)
        # feature_points = pts[0:2].T.astype('int32')
        return lines, lines_desc
    ...
    def reprocess_lines(self, junctions, heatmap, coarse_desc):
        ...
        junctions = torch.cat([junc_pred[0][..., None],
                                    junc_pred[1][..., None]], axis=-1)
    ...
    def keypoints_to_grid(self, keypoints, img_size):
        n_points = keypoints.size()[-2]
        device = keypoints.device
        grid_points = keypoints.float() * 2. / torch.tensor(
            [self.H, self.W], dtype=torch.float, device=device) - 1.
    ...
    def sample_line_points(self, line_seg):
        ...
        num_samples_lst = torch.clip(line_lengths // self.min_dist_pts,
                                  2, self.num_samples)
        line_points = torch.empty((num_lines, self.num_samples, 2), dtype=float)
        ...
        for n in np.arange(2, self.num_samples + 1):
            ...
            line_points_x = torch.linspace(cur_line_seg[:, 0, 0],
                                        cur_line_seg[:, 1, 0],
                                        n, axis=-1)
            line_points_y = torch.linspace(cur_line_seg[:, 0, 1],
                                        cur_line_seg[:, 1, 1],
                                        n, axis=-1)
            cur_line_points = torch.stack([line_points_x, line_points_y], axis=-1)
            ...
            cur_line_points = torch.cat([
                cur_line_points,
                torch.zeros((cur_num_lines, self.num_samples - n, 2), dtype=float)],
                axis=1)
