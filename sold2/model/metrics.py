...
class junction_pr(object):
    ...
    def __call__(self, junc_pred_raw, junc_gt, valid_mask):
        ...
        for thresh in list(self.intervals):
            ...
            tn = np.sum((junc_pred == 0).astype(np.float64)
                        * (junc_gt == 0).astype(np.float64) * valid_mask)
            fp = np.sum((junc_pred == 1).astype(np.float64)
                        * (junc_gt == 0).astype(np.float64) * valid_mask)
            fn = np.sum((junc_pred == 0).astype(np.float64)
                        * (junc_gt == 1).astype(np.float64) * valid_mask)
